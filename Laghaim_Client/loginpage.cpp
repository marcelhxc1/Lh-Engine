...
BOOL CLoginPage::ReadServerListData(char *filename) //0215
{
	if( g_dwClientCountry == CTRY_CN ) // 현재는 중국만 웹 서버리스트를 도입.
	{
		...
		{
		case CTRY_KOR:
			sprintf(address, "http://121.254.248.14/update2/"); // 테스트용.
			break;
		case CTRY_CN:
			sprintf(address, "http://laghaim-entry1.gamania.com.cn/");
			break;
		default:
			return FALSE;
		}
...
void CLoginPage::LoadRes()
{
	...
	m_pCheck_on->Xpos = m_pLoginBack->Xpos+143;
	m_pCheck_on->Ypos = m_pLoginBack->Ypos+314;

	m_pCheck_out->Xpos = m_pLoginBack->Xpos+143;
	m_pCheck_out->Ypos = m_pLoginBack->Ypos+314;

	m_ServerList->SetPosDim(m_pLoginBack->Xpos + 29, m_pLoginBack->Ypos + 55, 213, 198);

	m_LoginBtn.SetPosition( m_pLoginBack->Xpos+167, m_pLoginBack->Ypos+268 );     // 접속
	m_OptionBtn.SetPosition( m_pLoginBack->Xpos+63, m_pLoginBack->Ypos+354 );	    // 설정
	m_ExitBtn.SetPosition( m_pLoginBack->Xpos+136, m_pLoginBack->Ypos+354 );       // 종료



	m_pLoginBack->SetColorKey(RGB(255, 0, 255));
	m_pCheck_on->SetColorKey(RGB(255, 0, 255));
	m_pCheck_out->SetColorKey(RGB(255, 0, 255));
...
void CLoginPage::Draw2D()
{
	...
	if (m_bIsAniEnd && m_bAnimateBtn == FALSE)
	{
		...
		m_LoginBtn.Draw( m_pLoginBack->Xpos+167, m_pLoginBack->Ypos+268 );     // 접속
		m_OptionBtn.Draw( m_pLoginBack->Xpos+63, m_pLoginBack->Ypos+354 );	    // 설정
		m_ExitBtn.Draw( m_pLoginBack->Xpos+136, m_pLoginBack->Ypos+354 );       // 종료
		...
		m_IdBox.Draw(m_pLoginBack->Xpos+63, m_pLoginBack->Ypos+273 , 106 );
		m_PasswdBox.Draw(m_pLoginBack->Xpos+63, m_pLoginBack->Ypos+295, 106 );
...
void CLoginPage::MsgProc( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
	...
	if(m_GoHomePage.CheckMsgExist() == TRUE)
	{
		...
		{			
			...
			{
				ShellExecute(hWnd, NULL , "iexplore", "http://join.barunsonena.com/?call_type=MEMMOD", NULL, SW_SHOWNORMAL);
...
BOOL CLoginPage::LoginProcess(int nSvrIdx, char *strUserId, char *strUserPw, BOOL bAuto)
{
	...
	else if( ret == 0 )
	{
		...
		{
			m_GoHomePage.PutString("정보 보호를 위한 정보 수정이 필요합니다.지금 정보를 변경하시겠습니까?");
		}
	...
	{
		char buf[MAX_PATH] = {0,};
		sprintf(buf, "Laghaim [IP : %s][PORT : %d]", g_pTcpIp->GetIp().c_str(), g_pTcpIp->GetPort());		
