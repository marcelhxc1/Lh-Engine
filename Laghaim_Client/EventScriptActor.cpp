...
void CEventScriptActor::SceneChange( int SceneIndex, BOOL bNotCheckFade ) // 씬 전환 처리. 대사를 서피스로 만들고 웨이트 타임이나 웨이트 액션등을 처리해준다.
{
	...
	if( m_pNowScene->m_strContents )
	{
		...
		for( int i = 0 ; i < m_ContantsLineCount ; ++i )
		{
			...
			{
				g_pDisplay->CreateSurfaceFromText( &m_pContantsOutSurf[i], m_Font, m_ContantsText[i], RGB(255, 0, 255), RGB(255, 255, 255) );
				if( m_pContantsOutSurf[i] )
					m_pContantsOutSurf[i]->SetColorKey(RGB(255, 0, 255));
			}
		}
...
//	m_StopLine = 0;
	...
	*/
	...
	// 페이지 넘김용 클릭 서피스.
	g_pDisplay->CreateSurfaceFromText( &m_pClickSurf, m_Font, "CLICK!", RGB(255, 0, 255), RGB(0, 255, 0) );
	if( m_pClickSurf )
		m_pClickSurf->SetColorKey(RGB(255, 0, 255));
	g_pDisplay->CreateSurfaceFromText( &m_pClickOnSurf, m_Font, "CLICK!", RGB(255, 0, 255), RGB(255, 255, 0) );
	if( m_pClickOnSurf )
		m_pClickOnSurf->SetColorKey(RGB(255, 0, 255));
	...
	if( m_pNowScene->m_strSelection1 && strlen(m_pNowScene->m_strSelection1) )
	{
		...
		{
			/*
						g_pDisplay->CreateSurfaceFromText( &m_pSelection1Surf, m_Font, "CLICK!", RGB(255, 0, 255), RGB(0, 255, 0) );
						if( m_pSelection1Surf )
							m_pSelection1Surf->SetColorKey(RGB(255, 0, 255));
						g_pDisplay->CreateSurfaceFromText( &m_pSelectionOn1Surf, m_Font, "CLICK!", RGB(255, 0, 255), RGB(255, 255, 0) );
						if( m_pSelectionOn1Surf )
							m_pSelectionOn1Surf->SetColorKey(RGB(255, 0, 255));
			*/
		}
		...
		{
			g_pDisplay->CreateSurfaceFromText( &m_pSelection1Surf, m_Font, m_pNowScene->m_strSelection1, RGB(255, 0, 255), RGB(255, 255, 50) );
			if( m_pSelection1Surf )
				m_pSelection1Surf->SetColorKey(RGB(255, 0, 255));
			g_pDisplay->CreateSurfaceFromText( &m_pSelectionOn1Surf, m_Font, m_pNowScene->m_strSelection1, RGB(255, 0, 255), RGB(50, 255, 50) );
			if( m_pSelectionOn1Surf )
				m_pSelectionOn1Surf->SetColorKey(RGB(255, 0, 255));
		}
	...
	{
		g_pDisplay->CreateSurfaceFromText( &m_pSelection2Surf, m_Font, m_pNowScene->m_strSelection2, RGB(255, 0, 255), RGB(255, 255, 50) );
		if( m_pSelection2Surf )
			m_pSelection2Surf->SetColorKey(RGB(255, 0, 255));
		g_pDisplay->CreateSurfaceFromText( &m_pSelectionOn2Surf, m_Font, m_pNowScene->m_strSelection2, RGB(255, 0, 255), RGB(50, 255, 50) );
		if( m_pSelectionOn2Surf )
			m_pSelectionOn2Surf->SetColorKey(RGB(255, 0, 255));
	...
	{
		g_pDisplay->CreateSurfaceFromText( &m_pSelection3Surf, m_Font, m_pNowScene->m_strSelection3, RGB(255, 0, 255), RGB(255, 255, 50) );
		if( m_pSelection3Surf )
			m_pSelection3Surf->SetColorKey(RGB(255, 0, 255));
		g_pDisplay->CreateSurfaceFromText( &m_pSelectionOn3Surf, m_Font, m_pNowScene->m_strSelection3, RGB(255, 0, 255), RGB(50, 255, 50) );
		if( m_pSelectionOn3Surf )
			m_pSelectionOn3Surf->SetColorKey(RGB(255, 0, 255));
