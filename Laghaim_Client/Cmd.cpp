	...
	if (!strncmp(g_Buf, "c", 1))
	{
		...
		else
		{
			pMgr->AddPlayer( g_ii[0], g_Buf2, nShowMark, (float) x, (float) z, ( float ) y,
							 dir, g_ii[1], g_ii[2], g_ii[3], v_num, combat_state,
							 g_ii[4], g_ii[5], g_ii[6], g_ii[7], g_ii[8], g_ii[9],
							 g_Buf3, g_Buf4, g_ii[10], nGuildType ,nGuildIndex ); // 길드 등급 이름, 등급 인덱스 추가
		}
...
//			strcat(buf, &g_Buf[nSayto_len]);
	...
	else
	{
		...
		if( g_pNk2DFrame )
		{
			if( strstr( arg, "돈이 부족" ) && pCMyApp->m_pUIMgr->m_bAutoHunt )
		  ...
		  )
		{
			...
			if(g_ii[3]==1)//내구도가 1이면 0이란뜻.
				...
				pItem->m_Special = g_ii[3]; //충전
				 ...
				 || (pItem->m_Vnum == 3067 || pItem->m_Vnum == 3261) // 에이디아
			   )
...
//			g_pNk2DFrame->GetControlUserShop()->m_bOpened = TRUE; // ShowUserShop()에서 하도록 수정.
	...
	else if( !strcmp(g_Buf,"reg") ) // 아이템 넣기. 클라이언트에서 요청한것에대한 성공실패 답변,이걸 받은후에 실제로 옮겨준다.
	{
		...
		if( g_ii[0] ) // 성공
		{
			...
			arg = AnyOneArg( arg, g_Buf );
			g_ii[7] = atoi( g_Buf ); // 금괴

			arg = AnyOneArg( arg, g_Buf );
			g_ii[8] = atoi( g_Buf ); // 은괴

			arg = AnyOneArg( arg, g_Buf );
			g_ii[9] = atoi( g_Buf ); // 동괴

			arg = AnyOneArg( arg, g_Buf );
			g_ii[2] = atoi( g_Buf ); // 등록 가격

			arg = AnyOneArg( arg, g_Buf );
			g_ii[3] = atoi( g_Buf ); // 인벤 팩 번호.

			arg = AnyOneArg( arg, g_Buf );
			g_ii[4] = atoi( g_Buf ); // 등록 수량

			BOOL bSlotInResult = TRUE; // 스루면 정상
	...
	else if( !strcmp(g_Buf,"list") ) // 상점 물품 품목 받음.
	{
		...
		if(	g_SvrType == ST_ADULT_ONLY ) // [6/1/2007 Theodoric] 내구도 수리
		{
			g_pNk2DFrame->GetControlUserShop()->SetSlotItem( g_ii[0], g_ii[1], g_ii[2], g_ii[3], g_ii[4], g_ii[5], g_ii[6]
					, g_ii[12], g_ii[13], g_ii[9], g_ii[10], g_ii[11], g_ii[7], g_ii[8] ); // 아이템 정보를 셋팅.
		}
		else
		{
			g_pNk2DFrame->GetControlUserShop()->SetSlotItem( g_ii[0], g_ii[1], g_ii[2], g_ii[3], g_ii[4], g_ii[5], g_ii[6]
					, 0, 0 , g_ii[9], g_ii[10], g_ii[11], g_ii[7], g_ii[8]); // 아이템 정보를 셋팅.
		}
	}
...
void CmdMovedItem(char*arg, CUIMgr*pMgr)
{
	...
	if( rv == 0 )
	{
		...
		if( pMgr->m_AutoPlay && pMgr->m_AutoPlay->IsRun() )
		{
			...
			{
			case 1:
				pMgr->m_AutoPlay->AddLog( "*****퀵슬롯번호 오류*****" , 1 );
				break;
			case 2:
				pMgr->m_AutoPlay->AddLog( "*****팩번호 오류*****" , 1 );
				break;
			case 3:
				pMgr->m_AutoPlay->AddLog( "*****잘못된 아이템 인덱스*****" , 1 );
				break;
			}
		}
	}
...
void CmdShutdown(char*arg, CUIMgr*pMgr)
{
	...
	g_pNk2DFrame->InsertPopup( "셧다운제", TYPE_NOR_OK, 0 );
}
