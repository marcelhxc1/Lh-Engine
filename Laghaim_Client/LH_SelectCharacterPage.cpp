...
void LH_SelectCharacterPage::Action( int i )
{
	...
	case GOCREATE:
		{	
			...
			if( hour > 0 )
			{
				msg = boost::str(boost::format("캐릭터 생성 제한 남은시간 : %d시간 %d분 %d초") % hour % min % sec);
			}
			else if( min > 0 )
			{
				msg = boost::str(boost::format("캐릭터 생성 제한 남은시간 : %d분 %d초") % min % sec);
			}
			else
			{
				msg = boost::str(boost::format("캐릭터 생성 제한 남은시간 : %d초") % sec);
			}
