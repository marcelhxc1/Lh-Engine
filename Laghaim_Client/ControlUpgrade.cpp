...
const int NeedLaim_GodPower = 200000000;
			...
			if( pCheckItem )
			{
				if( GMRDia == 0)
				{
					...
					else
					{
						...
						case 3:
						case 7:
							// IDS_UPGRADE_MSG1 업그레이드 실패 시 초기화 될수 있습니다.
							g_pNk2DFrame->InsertPopup((char*)G_STRING(IDS_UPGRADE_MSG1), TYPE_NOR_OK, 1 );
							break;
						case 11:
							// IDS_UPGRADE_MSG2 업그레이드 실패 시 하락될 수 있습니다.
				...
				case INTERFACE_TYPE_ITEMMAKE:
					{
						...
						{
						case UPGRADE_ANI_SUCCESS:
							g_pNk2DFrame->InsertPopup( "아이템 제작 성공", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "아이템 제작 성공", -1); // 채팅창에도 남겨준다.
							break;

						case UPGRADE_ANI_FAIL:
							g_pNk2DFrame->InsertPopup( "아이템 제작 실패", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "아이템 제작 실패", -1); // 채팅창에도 남겨준다.
							break;
						}
					}
					break;
				...
				case INTERFACE_TYPE_BUFFMAKE_MAKE:
					{
						...
						{
						case UPGRADE_ANI_SUCCESS:
							g_pNk2DFrame->InsertPopup( "버프 아이템 제작 성공", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "버프 아이템 제작 성공", -1); // 채팅창에도 남겨준다.
							break;

						case UPGRADE_ANI_FAIL:
							g_pNk2DFrame->InsertPopup( "버프 아이템 제작 실패", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "버프 아이템 제작 실패", -1); // 채팅창에도 남겨준다.
							break;
						}
					}
					break;
				...
				case INTERFACE_TYPE_BUFFMAKE_GATCHA:
					{
						...
						{
						case UPGRADE_ANI_SUCCESS:
							g_pNk2DFrame->InsertPopup( "버프 아이템 갓챠 성공", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "버프 아이템 갓챠 성공", -1); // 채팅창에도 남겨준다.
							break;

						case UPGRADE_ANI_FAIL:
							g_pNk2DFrame->InsertPopup( "버프 아이템 갓챠 실패", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "버프 아이템 갓챠 실패", -1); // 채팅창에도 남겨준다.
							break;
						}
					}
					break;
				...
				case INTERFACE_TYPE_BUFFMAKE_MEDAL:
					{
						...
						{
						case UPGRADE_ANI_SUCCESS:
							g_pNk2DFrame->InsertPopup( "메달 교환 성공", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "메달 교환 성공", -1); // 채팅창에도 남겨준다.
							break;

						case UPGRADE_ANI_FAIL:
							g_pNk2DFrame->InsertPopup( "메달 교환 실패", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "메달 교환 실패", -1); // 채팅창에도 남겨준다.
							break;
						}
					}
					break;
				...
				case INTERFACE_TYPE_ITEMSEPARATE:
					{
						...
						{
						case UPGRADE_ANI_SUCCESS:
							g_pNk2DFrame->InsertPopup( "아이템 분해 성공", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "아이템 분해 성공", -1); // 채팅창에도 남겨준다.
							break;

						case UPGRADE_ANI_FAIL:
							g_pNk2DFrame->InsertPopup( "아이템 분해 실패", TYPE_NOR_OK, 32 );
							g_pNk2DFrame->AddChatStr( "아이템 분해 실패", -1); // 채팅창에도 남겨준다.
							break;
						}
					}
					break;
