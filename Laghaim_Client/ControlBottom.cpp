...
int CControlBottom::Draw(CItem **ppOverItem)
{
	...
	m_imgLetterNew.Draw();
	if( YutManager::GetInstance()->GetYutnoriOn() == true )
	{
		m_btn_Yutnori.DrawToBlt();
	}
	if( FishingManager::GetInstance()->GetFishingOn() == true )
	{
	...
	if( BaseballManager::GetInstance()->GetBaseballOn() == true )
	{
	...
	if( DiceGameManager::GetInstance()->GetDiceGameOn() == true )
	{
...
//	if( !m_chat_hidden_list )
	...
	case WM_LBUTTONDOWN:
		...
		if(!pCMyApp->IsSoccerZone())
		{
			...
			if( baseballMgr->GetBaseballOn() == true )
			{
			...
			if( dicegameMgr->GetDiceGameOn() == true )
			{
			...
			if( m_chat_size_big.MsgProc( hWnd, msg, wParam , lParam ) == S_OK )
			{
				...
				m_QlistBox->m_PosY = m_pChatOutOpt_Back->Ypos + 15 + ( 12 - m_QlistBox->m_ViewCount ) * 19;
	...
	case WM_KEYDOWN:
		...
		case VK_F8:
			...
			{
				AddStrQlist( "라그러쉬 설정중엔 스킬단축키 지정을 할수 없습니다." , -1 );
				return 0;
			}
			break;
		}
...
void CControlBottom::RegistChatProc()
{
	...
	REGIST_CHAT_PROC("평화",			ProcChat_Peace);
...
void CControlBottom::ProcChat_ControlTower(char* chat)
{
	...
	else if( strcmp("자동", buf) == 0 )
	{
		g_pTcpIp->SendNetMessage( "controltower on\n" );
	}
	else if( strcmp("수동", buf) == 0 )
	{
