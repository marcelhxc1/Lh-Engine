...
//==============================================
	...
	{
		m_MoneyTextKor1.SetString("(라임)", RGB(0, 0, 0), LAIM_KOR_BACK_COLOR);
		m_MoneyTextKor2.SetString("(라임)", RGB(0, 0, 0), LAIM_KOR_BACK_COLOR);
	}
...
void CControlExchange::LoadRes()
{
	...
	m_CancelBtn.SetPosition( bgx + 214 , bgy + 6 );
...
void CControlExchange::ExchMoney(char *strMoney)
{
	if (strlen(strMoney) < 11)
	{
		...
		m_MoneyText1.SetString(atoi(m_strMoney1), true);
		if(g_dwClientCountry ==CTRY_KOR)	//-- 한국만 지원
		{
			int curMoney = atoi(m_strMoney1);
...
char *ConvertMoneyToString( char *DestString, int Money, int SpaceSize, int Type ) // 돈 금액의 단위를 말로 바꿔주는 함수.
{
	...
	if( laim )
		sprintf(strMoneyText, "%s%d", strMoneyText, laim);
