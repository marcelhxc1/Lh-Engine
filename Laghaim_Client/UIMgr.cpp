...
void CUIMgr::CmdAttack(char *msg)
{
	...
	else	// npc 공격
	{
		...
		else if( pMob->m_nMobNum == 464 ) // 데카렌 인던 보스 ///인던1
		{
				...
				if (!strncmp(buf, "c", 1))
				{
					...
					if (ii[2] == g_pRoh->m_nCharIndex)
					{
						...
						if( pMob->m_nMobNum == 38 || pMob->m_nMobNum == 331
								|| pMob->m_nMobNum == 455 || pMob->m_nMobNum == 462 ) ///태국뮤턴트이벤트 요청으로 몬스터볼도 워자 미사일 쏜다.
			...
			case SKILL_PET_DOGS_BLACK_WHITE:
				{
					if( nSkill_OnOff == 1 )
					{
						...
						EffectSort ef_sort;
						ef_sort.nType = FX_FOLLOW; // 따라다니는 이펙트의
						ef_sort.Index = FX_FOLLOW_TYPE_DOGS; // 복돼지.
						ef_sort.subNum = skill_index; // 복돼지 종류. 총 4종.
...
BOOL CUIMgr::GetLagrushPayment()
{
	if( !m_bPaymentLagrush )
		g_pNk2DFrame->InsertPopup("라그러쉬 결제 후 이용이 가능합니다.",TYPE_NOR_OK, 1);
