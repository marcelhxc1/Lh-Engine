...
void CControlShop::SetMoney()
{
	if( m_pNkCha == NULL ) // 예외 처리.
		return;

	if( (m_shop_type&MASK_SHOP_PAY_TYPE) == SHOP_PAY_TYPE_LAIM ) // 라임상점일 경우
		m_MoneyText.SetString(m_pNkCha->m_Money,true);
	else if( (m_shop_type&MASK_SHOP_PAY_TYPE) == SHOP_PAY_TYPE_BP ) // BP상점일 경우
	{
		char TempString[200];
		sprintf( TempString, "%d BP", m_pNkCha->m_nBattlePoint/100 );
	...
	case WM_LBUTTONDOWN:
		...
		if (IsInside(x, y))
		{
			...
			else if (m_pNkCha && m_pNkCha->m_ExtraSlot && !(GET_ATT_FLAG(m_pNkCha->m_ExtraSlot) & IATT_NO_SELL) )
			{
				...
				int ResultMoney = m_pNkCha->m_Money + GET_PRICE(m_pNkCha->m_ExtraSlot); // 팔고 난후의 소지금.
			...
			if( m_shop_type & MASK_SP_SHOP )
			{
				...
				if (vnum >= 0)
				{
					...
					if( m_shop_type == 19 )
					{
						if( m_lpshop_wait )
						{
							g_pNk2DFrame->InsertPopup("잠시만 기다려 주십시요.", TYPE_NOR_OK, 1);
							return 0;
						}
				...
				if (vnum >= 0)
				{
					...
					else
					{
						if( g_dev_client )
						{
							...
							{
								g_pNk2DFrame->AddChatStr( "과징금 3배" , -1 );
							}
							else if( chaotic <= -500 )
							{
								g_pNk2DFrame->AddChatStr( "과징금 10배" , -1 );
							}
						}
	...
	case WM_RBUTTONDBLCLK:
		...
		if (x >= m_shopslotstart_x && y >= m_shopslotstart_y)
		{
					...
					( type == ITYPE_SUB && (shape == ISUB_COMPONENT) ) )
			{
				...
				{
					//
					g_pNk2DFrame->AddChatStr( "LP 상점은 낱개로만 구매 가능합니다.." , -1);
					return 0;
				}
...
BOOL CControlShop::CheckSellWarning( CItem *pItem, int PopupType ) // 팔때 경고 띄우기. (이 안에서 팝업 호출이 있음에 유의) ///다중판매 시 작업
{
	...
	{	
		if( CheckIsConversionItem(pItem,2)
			|| CheckIsConversionItem(pItem,1) )
		{
			g_pNk2DFrame->InsertPopup("고가의 아이템 입니다. 정말 파시겠습니까?",TYPE_NOR_OKCANCLE, PopupType);	// 5:POPUP_SHOP
			return 1;
		}
	}
