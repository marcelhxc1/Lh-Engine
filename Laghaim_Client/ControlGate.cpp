...
void CControlGate::SetCurWorld(int nCurWorldNum)
{
	...
	if (m_GateList == NULL)
	{
		m_GateList = new CListBox(" ", m_pSurfaceBack->Xpos+25, m_pSurfaceBack->Ypos+81, 191, 226);
		if( !m_GateList )
			return;
		...
		else if(nCurWorldNum == WORLD_MATRIX )
		{
			...
			else
			{
							 ...
							 || pCMyApp->m_pUIMgr->m_nWarpGateIndex == 164 ))
				{
					if( pCMyApp->m_pUIMgr->m_nWarpGateIndex != nIndex )
					{
						m_GateList->SetPosDim(m_pSurfaceBack->Xpos+25, m_pSurfaceBack->Ypos+81, 191, 226);
						m_GateList->LoadRes();
						return;
					}
				}
							...
							else if ( !g_pRoh->IsDead() )
							{
								...
								else if( pCMyApp->m_nCurWorldIdx == WORLD_MATRIX )
								{
									...
									if( m_GateType == 1010 )
									{
										...
										else if( g_bGlobalMatrix && sel == 2 )	// 국가별 그리니쉬 스퀘어 // 국가대항전 시에만 가능
										{
											if (g_pTcpIp)
											{
												pCMyApp->m_pUIMgr->m_bNextGoto = FALSE; // 대쉬 문제 등을 위한 체크 변수.
