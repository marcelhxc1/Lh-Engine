...
void CControlWareHouse::LoadRes()
{
	...
	{
		m_pStoreWidening_time_back->Xpos = m_pBack->Xpos + 64;
		m_pStoreWidening_time_back->Ypos = m_pBack->Ypos + 31;
	}	
	...
	m_InMoneyBtn.SetPosition(m_pBack->Xpos+142, m_pBack->Ypos+379);
	m_OutMoneyBtn.SetPosition(m_pBack->Xpos+196, m_pBack->Ypos+379);
	m_CloseBtn.SetPosition(m_pBack->Xpos+257, m_pBack->Ypos+4);
	m_BtnWindow_1.SetPosition(m_pBack->Xpos+20, m_pBack->Ypos+66);
	m_BtnWindow_2.SetPosition(m_pBack->Xpos+99, m_pBack->Ypos+66);
	m_BtnWindow_3.SetPosition(m_pBack->Xpos+178, m_pBack->Ypos+66);

	m_MoneyText.SetPos(m_pBack->Xpos+29, m_pBack->Ypos+381);
	...
	if( g_dwClientCountry == CTRY_TH || g_dwClientCountry == CTRY_JPN ) // #define CTRY_TH 7
	{
		...
		m_pCashViewBtn->SetPosition(m_pBack->Xpos+142, m_pBack->Ypos+405);
		m_pNormalViewBtn->SetPosition(m_pBack->Xpos+142, m_pBack->Ypos+405);
		...
		if(IsInside(x, y))
		{
				...
				if( timeGetTime() - BtnClickTick > 1000 )
				{
					...
					else // 일반 창고를 보고 있는 상태라면 유료상점 보기 버튼 처리
					{
						...
						if( m_pCashViewBtn->GetState() == BTN_DOWN )
						{
							...
							{
								//g_pNk2DFrame->InsertPopup( (char*)G_STRING(IDS_NEED_CASH_STASH_ITEM), TYPE_NOR_OK, 1 );
								g_pNk2DFrame->InsertPopup( "창고 확장 기간이 종료되었거나 활성화 되지 않았습니다.", TYPE_NOR_OK, 1 );
					...
					&& y < m_wareslot_start_y + WARE_SLOT_HEIGHT*WARE_SLOT_Y_NUM )
			{
				...
				else if (m_WareSlot[m_nPackNum][slot_x_num][slot_y_num] > 0)
				{
					...
					{
						if( g_dev_client )
							g_pNk2DFrame->AddChatStr( "찾기 불가" , -1 );
						return 1;
					}
	...
	case WM_LBUTTONUP:
		...
		if(IsInside(x, y))
		{
			...
			else if(m_OutMoneyBtn.GetState() == BTN_ACTION && m_OutMoneyBtn.IsInside(x,y))
			{
				...
				{
					if( g_dev_client )
						g_pNk2DFrame->AddChatStr("출금불가",-1);
					return 0;
				}
