...
//		m_lst_zone->AddString(G_STRING(IDS_GATE_WORLD_MOVE));
	...
	m_btn_close->SetPosition( bgx+219,bgy+5 );
	m_lst_zone->SetPosDim(bgx+25, bgy+81, 191, 226);
	m_lst_zone->LoadRes();
	...
	case WM_LBUTTONDOWN:
		{
			...
			if (m_lst_zone && (sel = m_lst_zone->GetMessage(hWnd, msg, wParam, lParam )) >= 0)
			{
				...
				if( m_slot_index == -1 )
				{
					...
					{
						g_pNk2DFrame->InsertPopup( "조건이 맞질 않아 이동할 수 없습니다." ,TYPE_NOR_OK, 1);
						return 2;
					}
...
void LHUI_Gate::SetZoneList(int value , bool direct)
{
	...
	m_lst_zone->SetPosDim(m_background->m_x+25, m_background->m_y+81, 191, 226);
	m_lst_zone->LoadRes();
}
