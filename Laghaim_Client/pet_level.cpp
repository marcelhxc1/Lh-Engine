...
void CPetLevel::LoadRes()
{
	...
	m_Combo1.InsertItem( "",13, RGB(100,200,200), RGB(255,255,255) );
	// IDS_STUDY : 교육
	m_Combo1.InsertItem( (char*)G_STRING(IDS_STUDY),13, RGB(100,200,200), RGB(255,255,255) );
	// IDS_EVOLUTION : 진화
	m_Combo1.InsertItem( (char*)G_STRING(IDS_EVOLUTION),13, RGB(100,200,200), RGB(255,255,255) );
	// IDS_SKILL : 스킬
	m_Combo1.InsertItem( (char*)G_STRING(IDS_SKILL),13, RGB(100,200,200), RGB(255,255,255) );
	// IDS_PET_SKILL_INIT : 스킬 초기화
	m_Combo1.InsertItem( (char*)G_STRING(IDS_PET_SKILL_INIT), 13, RGB(100,200,200), RGB(255,255,255) );
	...
	case WM_MOUSEMOVE:
		...
		if( m_Combo1.MsgProc(hWnd, msg, wParam, lParam) )
		{
			...
			else if( m_Combo1.GetSelect() == 2 ) // 진화
			{
				...
				if( g_MyPetStatus.m_Class == 0 )
				{
					m_Combo2.InsertItem( "",13, RGB(100,200,200), RGB(255,255,255) );
					// IDS_DEFENCE_TYPE : 방어형
					m_Combo2.InsertItem( (char*)G_STRING(IDS_DEFENCE_TYPE),13, RGB(100,200,200), RGB(255,255,255) );
					// IDS_ATTACK_TYPE : 공격형
					m_Combo2.InsertItem( (char*)G_STRING(IDS_ATTACK_TYPE),13, RGB(100,200,200), RGB(255,255,255) );
				}
				else
				{
			...
			else if( m_Combo1.GetSelect() == 3 ) // 스킬
			{
				...
				case 1: // 방어형
					...
					m_Combo2.InsertItem( tmp,13, RGB(100,200,200), RGB(255,255,255) );
		...
		if(m_pBtn1 && m_pBtn1->GetState() == BTN_ACTION) // 승인
		{
			...
			else if( m_Combo1.GetSelect() == 2 ) // 진화
			{
				...
				{
					// 신석
					int cnt = g_pRoh->GetCountInExch(3331);
					if( cnt < 1 )
					{
					...
					// 빛나는 신석
					cnt = g_pRoh->GetCountInExch(3332);
					if( cnt < 1 )
					{
						g_pNk2DFrame->InsertPopup( (char*)G_STRING(IDS_NEWPET_MSG0), TYPE_NOR_OK, 1 );
						return 1;
					...
					// 오염된 사막 상자
					cnt = g_pRoh->GetCountInExch(3845);
					if( cnt < 10 )
					{
						g_pNk2DFrame->InsertPopup( (char*)G_STRING(IDS_NEWPET_MSG0), TYPE_NOR_OK, 1 );
						return 1;
...
void CPetLevel::SetMoney(int money)
{
	if(!m_pMoneyText)
		return;
