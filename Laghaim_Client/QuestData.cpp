...
const char * QUEST_DATA_FILENAME = "data/quest_table.txt";
const char * QUEST_DATA_FILENAME2= "data/quest_content_table.txt";
...
void CQuestData::PutString(int idx, char* String, int x, int y)
{
	...
	if (let > 0)
	{
		...
		m_pTextSur[idx]->DrawText(m_Font, String, 0, 0, RGB(255, 0, 255), RGB(231, 203, 102),0);
		m_pTextSur[idx]->SetColorKey(RGB(255, 0, 255));
...
BOOL CQuestData::QuestMessage() //1203 레벨업시에 나타나는 도움말 창.
{
	...
	if(!g_pNk2DFrame->IsInterfaceVisible(QUEST) || m_bReload)
	{
		...
		if(m_bScroll)
		{
			...
			m_pBarSur->SetColorKey(RGB(255, 0, 255));
