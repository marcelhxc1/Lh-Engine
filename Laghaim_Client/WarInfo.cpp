...
void CWarInfo::Draw()
{
	  ...
	  )
	{
		...
		//m_WarInfoBox[ 0 ].Draw( (iClintWidth/2) - (m_pWarInfoBase->GetWidth()/2)+20, 45 ); // 남은 시간
		//m_WarInfoBox[ 4 ].Draw( (iClintWidth/2) - (m_pWarInfoBase->GetWidth()/2)+25, 10 ); // 개인 점수 표시
		//m_WarInfoBox[ 0 ].Draw( MATSCOREBASEWIDTH+20, 45 ); // 남은 시간
		//m_WarInfoBox[ 4 ].Draw( MATSCOREBASEWIDTH+25, 10 ); // 개인 점수 표시
		m_WarInfoBox[ 0 ].Draw( WARINFO_BASE_X+20, WARINFO_BASE_Y+45 ); // 남은 시간
		m_WarInfoBox[ 4 ].Draw( WARINFO_BASE_X+25, WARINFO_BASE_Y+10 ); // 개인 점수 표시

		if( pCMyApp->m_nCurWorldIdx == 10 )
		{
			// 자신이 소속된 연합의 점수는 노란색으로 표시
