...
void CControlParty::LoadRes()
{
	...
	m_ExpGetRule.InsertItem( (char*)G_STRING(IDS_PARTY_EXP_RULE), 13, RGB(100,200,200), RGB(255,255,255) );
	//-- IDS_PARTY_EXP_RULE_DAMAGE : 데미지
	m_ExpGetRule.InsertItem( (char*)G_STRING(IDS_PARTY_EXP_RULE_DAMAGE), 13, RGB(100,200,200), RGB(255,255,255) );
	//-- IDS_PARTY_EXP_RULE_EQUAL : 균등
	m_ExpGetRule.InsertItem( (char*)G_STRING(IDS_PARTY_EXP_RULE_EQUAL), 13, RGB(100,200,200), RGB(255,255,255) );
	...
	m_ItemGetRule.InsertItem( (char*)G_STRING(IDS_PARTY_ITEM_RULE), 12, RGB(100,200,200), RGB(255,255,255) );
	//-- IDS_PARTY_ITEM_RULE_DAMAGE : 데미지 우선
	m_ItemGetRule.InsertItem( (char*)G_STRING(IDS_PARTY_ITEM_RULE_DAMAGE), 12, RGB(100,200,200), RGB(255,255,255) );
	//-- IDS_PARTY_ITEM_RULE_FIRSTGET : 입수 우선
	m_ItemGetRule.InsertItem( (char*)G_STRING(IDS_PARTY_ITEM_RULE_FIRSTGET), 12, RGB(100,200,200), RGB(255,255,255) );
	//-- IDS_PARTY_ITEM_RULE_RANDOM : 무작위
	m_ItemGetRule.InsertItem( (char*)G_STRING(IDS_PARTY_ITEM_RULE_RANDOM),12, RGB(100,200,200), RGB(255,255,255) );
	//-- IDS_PARTY_ITEM_RULE_LEADER : 파티장 우선
	m_ItemGetRule.InsertItem( (char*)G_STRING(IDS_PARTY_ITEM_RULE_LEADER),12, RGB(100,200,200), RGB(255,255,255) );
	//-- IDS_PARTY_ITEM_RULE_TURN : 순차
	m_ItemGetRule.InsertItem( (char*)G_STRING(IDS_PARTY_ITEM_RULE_TURN),12, RGB(100,200,200), RGB(255,255,255) );
	...
	for( int i = 0 ; i < MAX_PARTY_NUM_BOSSRAID_FINAL; i++ )
	{
		if( m_aPartyList[i].m_Name ) // 이름이 있는 녀석만 복구.
		{
			...
			g_pDisplay->CreateSurfaceFromText( &m_aPartyList[i].m_pSurName
											   , m_Font, m_aPartyList[i].m_Name, RGB(255, 0, 255), RGB(255,255,0) );
			if( m_aPartyList[i].m_pSurName )
				m_aPartyList[i].m_pSurName->SetColorKey(RGB(255, 0, 255));
		}
	}
...
void CControlParty::AddMember(int char_idx, char *name)
{
	...
	g_pDisplay->CreateSurfaceFromText( &m_aPartyList[m_MemberCount].m_pSurName
									   , m_Font, m_aPartyList[m_MemberCount].m_Name, RGB(255, 0, 255), RGB(255,255,0) );
	if( m_aPartyList[m_MemberCount].m_pSurName )
		m_aPartyList[m_MemberCount].m_pSurName->SetColorKey(RGB(255, 0, 255));
...
void CControlParty::SetPartyCount()
{
...

	{
		...
		g_pDisplay->CreateSurfaceFromText( &m_pPartyCountSur, m_Font, str, RGB(255, 0, 255), RGB(255,255,255) );
		m_pPartyCountSur->SetColorKey(RGB(255, 0, 255));
