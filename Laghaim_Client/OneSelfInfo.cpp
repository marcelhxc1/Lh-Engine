...
void COneSelfInfo::LoadRes()
{
	...
	m_pBackSur->SetColorKey(RGB(255, 0, 255));
...
void COneSelfInfo::PutString(int index, char* String)
{
	...
	if( m_pTextSur[index] )
	{
		m_pTextSur[index]->DrawText(m_Font, m_SaveBuf[index], 0, 0, RGB(255, 0, 255), RGB(255, 228, 132), 0);
		m_pTextSur[index]->SetColorKey(RGB(255, 0, 255));
	}
}
...
void COneSelfInfo::RestoreAll()
{
	...
	for(int i = 0; i<TEXTSURFACE_CNT; i++)
	{
		...
		if (let > 0)
		{
			...
			if( m_pTextSur[i] )
			{
				m_pTextSur[i]->DrawText(m_Font, m_SaveBuf[i], 0, 0, RGB(255, 0, 255), RGB(255, 210, 255), 0);
				m_pTextSur[i]->SetColorKey(RGB(255, 0, 255));
			}
		}
	}
...
// 보유금액
void COneSelfInfo::Set_CurMoney(int curmoney)
{
	_itoa( curmoney, m_Tmp, 10 );
...
// 총경험치
void COneSelfInfo::Set_CurExp(int exp)
{
	_itoa( exp, m_Tmp, 10 );
...
// 레벨업 까지 남은 경험치
void COneSelfInfo::Set_NeedExp(int needexp)
{
	_itoa( needexp, m_Tmp, 10 );
