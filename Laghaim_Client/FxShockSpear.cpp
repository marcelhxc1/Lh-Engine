...
CFxShockSpear::CFxShockSpear()
{
	m_nTotalFrame = FX_SHOCKSPEAR_FRAMECOUNT;
...
void CFxShockSpear::LoadRes(IndexedTexture *pIndexedTexture, CShadowRect *pShadowRect)
{}
void CFxShockSpear::DeleteRes()
{}

void CFxShockSpear::RenderRing(EffectSort &effect_sort)
{
	if( !GET_D3DDEVICE() )
		return;
	...
	{
		MessageBox(NULL, "프레임카운트가 넘게 들어왔네요", "Test", MB_OK);
		return;
	}
...
void CFxShockSpear::RenderBoom(EffectSort &effect_sort)
{
	if( !GET_D3DDEVICE() )
		return;
...
void CFxShockSpear::RenderLight(EffectSort &effect_sort)
{
	if( !GET_D3DDEVICE() )
		return;
...
void CFxShockSpear::Render(EffectSort &effect_sort)
{
	if( !GET_D3DDEVICE() )
		return;
