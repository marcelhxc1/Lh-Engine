...
public:
	...
	BOOL SlotIn( int SlotPosNum, int GoldLime, int SilverLime, int BronzeLime, int Price, int InvenPack, int InvenX, int InvenY ); // 인벤에서 상점 슬롯으로 넣음.(한개를 넣어주는 함수다. 패킷을 받은 시점에서 카운트 만큼 이 함수가 호출된다.)(실질적으로 서버에서 받은 이후의 처리임.)
...
//	BOOL SlotOut( int SlotPosNum, int InvenPack, int InvenX, int InvenY ); // 상점 슬롯에서 인벤으로 넣음.(한개를 넣어주는 함수다. 패킷을 받은 시점에서 카운트 만큼 이 함수가 호출된다.)(실질적으로 서버에서 받은 이후의 처리임.)
	...
	void SelledItem( int SlotPosNum, int TotalSelledGoldLime, int TotalSelledSilverLime
