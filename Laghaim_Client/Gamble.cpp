...
void CGamble::LoadGamble()
{
	...
	if(m_bCooSuk)
	{
		...
		if( m_pBinggo )
			m_pBinggo->SetColorKey(RGB(255, 0, 255));
		if( m_pBack_Yut )
		{
			{
		...
		if (m_pAlphaPopUp->GetIndex() != -100)
		{
			...
			else
			{
				if (m_bType == _TYPE_GAMBLE2)
				{
					...
					{
						multiText.AddString("동일한 종류의 매직 보석류 아이템을 1개이상(최대12개)", _FCOLOR_GREEN);
						multiText.AddString("올려놓고 1~6의 버튼 중 하나를 선택하여 당첨되면 2배를", _FCOLOR_GREEN);
						multiText.AddString("획득합니다. 이용 시 10LP가 소모됩니다", _FCOLOR_GREEN);
					}
					else
					{	
