...
extern char *ConvertMoneyToString( char *DestString, int Money, int SpaceSize, int Type = 0 ); // 돈 금액의 단위를 말로 바꿔주는 함수.
		...
		else if( m_iType != USHOP_TYPE_BUYER ) // 개인상점 슬롯이 아니면 인벤을 체크해본다. // 단, 구매자는 무시.
		{
			...
			if( pRefInven && pRefInven->IsInside(x, y) && !m_bSlotLock ) // 인벤쪽 슬롯을 더블클릭한 경우.
			{
				...
				if( (GET_ATT_FLAG(pItem) & IATT_NO_EXCHANGE) || pItem->m_bTimeLimit  // 교환 불가 아이템은 못팔도록.
						|| pItem->m_Vnum == 1336 || pItem->m_Vnum == 1337 || pItem->m_Vnum == 1338  // 금,은,동괴는 못판다.
				...
				if( m_iType != USHOP_TYPE_BUYER && !m_bSlotLock && msg == WM_LBUTTONDOWN ) // 물건을 등록하거나 뺄 수있는 경우다. ///점포상점 시 수정
				{
					...
					if (g_pRoh && g_pRoh->m_ExtraSlot)
					{
						if( (GET_ATT_FLAG(g_pRoh->m_ExtraSlot) & IATT_NO_EXCHANGE) || g_pRoh->m_ExtraSlot->m_bTimeLimit  // 교환 불가 아이템은 못팔도록.
								|| g_pRoh->m_ExtraSlot->m_Vnum == 1336 || g_pRoh->m_ExtraSlot->m_Vnum == 1337 || g_pRoh->m_ExtraSlot->m_Vnum == 1338  // 금,은,동괴는 못판다.
...
BOOL CControlUserShop::SlotIn( int SlotPosNum, int GoldLime, int SilverLime, int BronzeLime, int Price, int InvenPack, int InvenX, int InvenY ) // 인벤에서 상점 슬롯으로 넣음.(한개를 넣어주는 함수다. 패킷을 받은 시점에서 카운트 만큼 이 함수가 호출된다.)(실질적으로 서버에서 받은 이후의 처리임.)
{
...
void CControlUserShop::SetSlotItem( int SlotPosNum, int vnum, int plus, int special, int special2,
									...
									int GoldLime, int SilverLime, int BronzeLime, int price, int count) // 아이템 정보를 셋팅.
{
	int x = 0, y = 0, pack_num = 0, slot_num = 0;
...
BOOL CControlUserShop::CheckMoneyStr(CMsgPopUp* pPopup)
{
	...
	int itemCount = nGold + nSilver + nBronze;
...
void CControlUserShop::SelledItem( int SlotPosNum, int TotalSelledGoldLime, int TotalSelledSilverLime
								   , int TotalSelledBronzeLime, __int64 TotalSelledMoney, int SelledCount )
{
	int x = 0, y = 0, pack_num = 0, slot_num = 0;
	char tmp[10] = ""; // 카운트 입력용 스트링 버퍼.
	...
	if( (m_SelledGoldLime!=TotalSelledGoldLime) || (m_SelledSilverLime!=TotalSelledSilverLime)
			|| (m_SelledBronzeLime!=TotalSelledBronzeLime) || (m_SelledLime!=TotalSelledMoney) ) // 바뀐 수치가 있다면...
	{
		CMultiText multiText;
		char strTemp[256];
		char strTemp2[256];
		...
		int GoldLime_price = 0, SileverLime_Price = 0, BronzeLime_Price = 0, proto_num = 0; // 금괴,은괴,동괴의 가격을 알아오기위해 필요한 녀석들
		...
		TempMoney += ((__int64)TotalSelledGoldLime*(__int64)GoldLime_price) + ((__int64)TotalSelledSilverLime*(__int64)SileverLime_Price)
					 + ((__int64)TotalSelledBronzeLime*(__int64)BronzeLime_Price) + (__int64)TotalSelledMoney;
...
HRESULT CControlUserShop::PopupMsg(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam, int nMsg, CMsgPopUp* pArgPopup)
{
	if( nMsg == 1 ) // 확인이 눌린경우.
	{
		...
		case 2: // 등록 확인.
			...
			// 서버로 패킷을 보낸다.
			sprintf( commOutBuf, "u_shop reg %d %d %d %d %d %d %d %d\n", m_TempMoveInfo.SlotIndex
					 , m_TempMoveInfo.InvenPack, m_TempMoveInfo.GoldCount, m_TempMoveInfo.SilverCount
		...
		if( CheckMoneyStr(pArgPopup) ) // 제대로 된 값이 넣어진 경우
		{
			...
			if( m_pRefTempExtraItem )
			{
				...
				if( m_TempMoveInfo.GoldCount == 0 && m_TempMoveInfo.SilverCount == 0
						&& m_TempMoveInfo.CopperCount == 0 && m_TempMoveInfo.Price < SELL_MIN_LIMIT ) // 특정 금액 이하면 물품 등록이 안되도록...
				{
				...
				sprintf( strTemp, (char*)G_STRING(IDS_USHOP_SELL_CHECK)
						 , GET_ITEM_NAME_VNUM(m_pRefTempExtraItem->m_Vnum), m_TempMoveInfo.GoldCount
						 , m_TempMoveInfo.SilverCount, m_TempMoveInfo.CopperCount, m_TempMoveInfo.Price );
