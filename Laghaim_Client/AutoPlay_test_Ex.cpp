...
// 		}
		...
		AddLog( "[_OnFieldProc:공격중:공격시도...]" , 1 );
...
void CAutoPlay_EX::_Polling_Full(DWORD dt)
{
	...
	{
		AddLog( "[_Polling_Full] Player Dead : 상태변경 요청" , 1 );
		m_dead_time = 0;
		pCMyApp->StopHero();
	...
	if( m_current_state == REVIVAL )
	{
		...
		{
			AddLog( "[_Polling_Full] Player Dead : 부활요청" , 1 );
			m_dead_time = 0;
		...
		{
			AddLog( "[_Polling_Full] Player Dead : 부활요청 대기중" , 1 );
		}
	...
	if( AVOID == m_current_state )
	{
		...
		{
			AddLog( "[_Polling_Full] Player Avoid : 회피시간끝 상태변경" , 1 );
			m_dwAvoidCurTime = 0;
			_ChangeState( NONE_STATE );
		}
		else
		{
			...
			avoid_tick = 0;
			AddLog( "[_Polling_Full] Player Avoid : 회피중" , 1 );
		}
		return;
	}
	...
	{
		AddLog( "[_Polling_Full:마을로복귀시도]" , 1 );
		_ChangeState( NONE_STATE );
		return;
	}
	...
	if( GO_TO_AREA_ON_WALK == m_current_state )
	{
		...
		AddLog( "[_Polling_Full:걸어서이동 도착]" , 1 );
		_ChangeState( NONE_STATE );
		return;
	}
	...
	if( _IsPeaceArea() )
	{
		if( !_OnTownProc(dt) )
		{
			AddLog( "[_Polling_Full:_OnTownProc] 실패" , 1 );
			_ChangeState( NONE_STATE );
		}
	}
	else
	{
		if( !_OnFieldProc(dt) )
		{
			AddLog( "[_Polling_Full:_OnFieldProc] 실패" , 1 );
			_ChangeState( NONE_STATE );
		}
	}
}
...
bool CAutoPlay_EX::_OnTownProc(DWORD dt)
{
	...
	else if( WARE == m_current_state )
	{
		...
		{
			AddLog( "[_OnTownProc:창고이용중]" , 1 );
			return true;
		}

		AddLog( "[_OnTownProc:상점이용변경]" , 1 );
	...
	else if( SHOP_BUY == m_current_state )
	{
		...
		{
			AddLog( "[_OnTownProc:상점이용중]" , 1 );
			return true;
		}

		if( true == _OnPotion_Town(dt) )
		{
			AddLog( "[_OnTownProc:HP 채우기 상점이용으로 변경]" , 1 );
...
bool CAutoPlay_EX::_OnFieldProc(DWORD dt)
{
	if( _OnAvoid(dt) )
	{
		AddLog( "[_OnFieldProc:회피에의한 귀환요청]" , 1 );
	...
	if( ITEM == m_current_state )
	{
		if( _OnFieldUsing(dt) )
		{
			AddLog( "[_OnFieldProc:아이템습득중:기본프로세스처리중]" , 1 );
			return true;
		}
		...
		if( !_OnPickItem(dt) )
		{
			...
			{
				AddLog( "[_OnFieldProc:아이템습득실패(대기시간)]" , 1 );
				_ChangeState( NONE_STATE );
			}
			else
			{
				AddLog( "[_OnFieldProc:아이템습득실패(이동중)]" , 1 );
			}
		}
		return true;
	}
	else if( ATTACK == m_current_state )
	{
		...
		if( rv_lost == -1 )
		{
			AddLog( "\t[_OnFieldProc] 퀵슬롯 오류 인벤에도 아이템 없다." );
		}
		else if( rv_lost == 0 )
		{
			// 정상
		...
		{
			AddLog( "\t[_OnFieldProc] 퀵슬롯 오류 저장된 아이템 이동요청." );
			return true;
		}

		if( _OnFieldUsing(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:기본프로세스처리중]" , 1 );
			return true;
		}

		if( _OnPet(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:펫관련처리성공]" , 1 );
			return true;
		}
		if( _OnSubArms(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:보조충전제처리성공]" , 1 );
			return true;
		}
		if( _OnAdiaSummon(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:에이디아소환처리성공]" , 1 );
			return true;
		}
		if( _OnShield(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:쉴드처리성공]" , 1 );
			return true;
		}
		if( _OnDsystem(dt) )
		{
		...
		if( _OnShield2(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:쉴드처리성공2]" , 1 );
			return true;
		}
		// 단축키 지정필요
		if( _OnShield3(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:쉴드처리성공3]" , 1 );
			return true;
		}
		if( _OnShield4(dt) )
		{
		...
		if( _OnRecovery(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:리커버리처리성공]" , 1 );
			return true;
		}
		if( _OnBloody(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:블러디마나처리성공]" , 1 );
			return true;
		}
		...
		{
			AddLog( "[_OnFieldProc:공격중:아이템발견]" , 1 );
			return true;
		}
		...
		{
			AddLog( "[_OnFieldProc:공격중:타겟결정 실패]" , 1 );
			return true;
		}

		if( _OnVampTouch(dt) )
		{
			AddLog( "[_OnFieldProc:공격중:뱀파이어터치처리성공]" , 1 );
			return true;
		}
		...
		if( !_OnAttack(dt) )
		{
			...
			{
				AddLog( "[_OnFieldProc:공격중:공격시도:5번실패...]" , 1 );
				_ChangeState( NONE_STATE );
				m_attack_try_count = 0;
				return false;
			}
	...
	else if( RETURN_TO_BASE == m_current_state )
	{
		if( _OnFieldUsing(dt) )
		{
			AddLog( "[_OnFieldProc:공격시작점복귀:기본프로세스처리중]" , 1 );
			return true;
		}

		if( _OnMoveToBase(dt) )
		{
			AddLog( "[_OnFieldProc:공격시작점복귀:복귀완료 공격모드로 변경]" , 1 );
			_ChangeState( ATTACK );
		}
...
State_ReturnValue CAutoPlay_EX::_OnFindNpc(DWORD dt, bool ware)
{
	...
	if( NULL == m_pTemporaryMob )
	{
		...
		for( prMob = pCMyApp->m_pUIMgr->m_mob_map.begin(); prMob != pCMyApp->m_pUIMgr->m_mob_map.end(); prMob++ )
		{
			...
			if( pMob->m_nMobNum == 666 && strstr(pMob->GetMobName(), "러시" ) )
			{
		...
		{
			_AddChat_Debug( "NPC를 찾아야되는데 몹도없고 캐릭도없다" );
...
State_ReturnValue CAutoPlay_EX::_OnStashItem(DWORD dt)
{
	...
	{
		if( g_dev_client )
			g_pNk2DFrame->AddChatStr( "[라그러쉬] 찾기 불가" , -1 );
		return State_RV__None;
	}
				...
				if( inven_item = g_pRoh->m_Inven[pack] )
				{
					while( inven_item )
					{
						...
						if( need_put )
						{
							...
							LH_SEND_NET_MSG( msg_buf );
							m_stash_sendmsg = 1;
							AddLog( "[_OnStashItem:창고맡기기 요청]" , 1 );
...
// 								break;
			...
			AddLog( "[_OnStashItem:창고맡기기완료:찾기로 변경]" , 1 );
...
bool CAutoPlay_EX::_OnAttack(DWORD dt)
{
	...
	if( !_IsSpecialMob(m_pSelectedMob->m_nMobNum) )
	{
		...
		case LR_Warning__InRange:
			_ChangeState(ATTACK);
			_AddChat_Debug( "선타격 몬스터를 발견하였습니다.(범위내)" );
			return false;
		case LR_Warning__OutRange:
			_ChangeState(RETURN_TO_BASE);
			_AddChat_Debug( "선타격 몬스터를 발견하였습니다.(범위밖)" );
			return false;
		}
	}
	...
	if( dist > m_fStayRange )
	{
		_AddChat_Debug( "공격중 몹이 범위 밖으러 나갔다." );
		_ChangeState( NONE_STATE );
		return false;
	}
	...
	{
		_AddChat_Debug( "공격중 캐릭터가 범위 밖으로 나갔다." );
		_ChangeState( RETURN_TO_BASE );
		return false;
	}
...
// 				temp_dist *= 0.5f;
			...
			sprintf( buf , "공격클릭 : 모션스텐드 일단 살짝이동해보자" );
			_AddChat_Debug( buf );
			AddLog(buf);
		...
		{
			sprintf( buf , "공격클릭 : In Water...change state" );
			_AddChat_Debug( buf );
			_ChangeState( RETURN_TO_BASE );
			return false;
		}
	...
	if( pCMyApp->m_pUIMgr->m_b_MovingForAttack )
	{
		bool try_attack = pCMyApp->m_pUIMgr->CmdClick( 0 , 0 , mx , my );
		char buf[128];
		sprintf( buf , "공격이동중클릭 : %d , %d" , try_attack , g_pRoh->GetMotionState() );
		_AddChat_Debug( buf );
		AddLog(buf);
		return try_attack;
	}
					...
					{
						{
							...
							char buf[128];
							sprintf( buf , "디바인관련 : %d" , try_attack );
							_AddChat_Debug( buf );
							AddLog(buf);
						}
...
bool CAutoPlay_EX::_OnDecideTarget(DWORD dt)
{
	...
	if( pCMyApp->m_pUIMgr->m_mob_map.empty() )
	{
		...
		else
		{
			...
			int my = 0;
			{
				...
				AddLog("[DecideTarget] : 몹이 없어서 3초마다 한번씩 살짝 이동");
				_OnMove(dt , g_pRoh->m_wx + mx, g_pRoh->m_wz + my );
			}
			return false;
		}
	...
	_AddChat_Debug( "몹은 있는데 범위 밖이거나...장애물때문에 이동을 못한다.\n" );
	AddLog( "[DecideTarget] : 몹은 있는데 범위 밖이거나...장애물때문에 이동을 못한다" );
...
bool CAutoPlay_EX::_OnShield(DWORD dt)
{
	...
	{
		m_bShield = false;
		_AddChat( "잘못된 스킬이거나 장비착용이 필요합니다. " );
		_AddChat( "쉴드사용을 제한합니다." );
		return false;
	}
...
UINT CAutoPlay_EX::PassMsg(UINT msg, UINT wparam, UINT lparam)
{
	...
	if( msg == WM_KEYDOWN )
	{
		...
		if( wparam == VK_ADD )
		{
			...
			float _x, _z;
			pCMyApp->m_pUIMgr->ConvertWorldPos(g_pRoh->m_wx, g_pRoh->m_wz , _x, _z );
			sprintf( szMessage, "%d번째 좌표 %d  %d 저장합니다.", num, (int)_x, (int)_z );
			_AddChat( szMessage );
			SaveWalkPosList( g_pRoh->m_wx, g_pRoh->m_wz );
		...
		else if( wparam == VK_SUBTRACT )
		{
			...
			int num = m_pairPoslist.size();
			pCMyApp->m_pUIMgr->ConvertWorldPos( ((POSPAIR)*(last_itr)).first , ((POSPAIR)*(last_itr)).second , _x, _z );
			sprintf( szMessage, "%d번째 좌표 %d  %d 삭제합니다.", num, (int)_x, (int)_z );
			_AddChat( szMessage );
			m_pairPoslist.erase( last_itr );
			SaveWalkPosList();
...
bool CAutoPlay_EX::CheckEquipSkill(int cur_skill)
{
	...
	case SKILL_A_LIFE_EXPANSION:
		...
		if( GET_ITEM_NUM( g_pRoh->m_Wearing[ WEARING_SHIELD ] ) != 1022 		// 1022 : 마술피리
				&& GET_ITEM_NUM( g_pRoh->m_Wearing[ WEARING_SHIELD ] ) != 3067 )	// 1022 : 마술피리
			return false;
		return true;
...
//		pair_slot[m_dwSpecialPotionKey[1]]++;
	...
	for( i=0 ; i<AH_QUICKSLOT_COUNT ; i++ )
	{
		if( pair_slot[i] > 2 )
		{
			sprintf(err_buf , "퀵슬롯 지정 %d 키가 중복되어 있습니다." , i+1);
			_AddChat( err_buf );
			return false;
		}
	}
	...
	if( m_bPetActive )
	{
		...
		{
			_AddChat( "라비용이 없습니다. " );
			_AddChat( "라비용사용을 중단합니다." );
			_AddChat( "사과구입을 중단합니다." );
			_AddChat( "호출기 구입을 중단합니다." );
			_AddChat( "호출기 사용을 중단합니다." );
