...
void CControlPetStatus::SetPetStatus(int type, int val, int val2)
{
	char tmp[30];
	int value = 0;
	int plus = 0;
	if(type == 0)  // PET_VITAL{
	{
		...
		sprintf(tmp, "%d/%d(%d)", val, value, val2);
	...
	else if(type == 1)  //PET_DAM_RANGE
	{
		...
		sprintf(tmp, "%d(%d)%s%d", value, val, ((plus >= 0) ? "+" : ""), plus);
