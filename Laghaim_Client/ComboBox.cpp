...
// 사이즈 셋팅은 이미지 로드 전에 이루어져야한다.
{
	...
	if( m_pListViewBtn )
	{
		...
		MessageBox( NULL, "메모리 부족! in ComboBox LoadRes", "Error", MB_OK );
	...
	else
	{
		...
		if( m_pSelectedBackGround )
			m_pSelectedBackGround->SetColorKey( RGB(255, 0, 255) );
		else
		{
	...
	else
	{
		...
		if( m_pListBackGround )
			m_pListBackGround->SetColorKey( RGB(255, 0, 255) );
		else
		{
...
int CComboBox::InsertItem( char* ItemText, int FontSize, DWORD TextColor, DWORD OverColor, DWORD BackColor )
{
	...
	{
		MessageBox( NULL, "콤보 박스에 넣을 수 있는 한도를 넘었습니다.", "Combo Box Error", MB_OK );
		return -1;
	}
	...
	{
		MessageBox( NULL, "메모리 부족! in ComboBox InsertItem", "Error", MB_OK );
		return -1;
	}
...
int CComboBox::InsertItemByImage( char* ImageFileName ) // 이미지로 삽입.
{
	...
	if( m_pListItem[m_InsertedCount] )
		m_pListItem[m_InsertedCount]->SetColorKey( RGB(255, 0, 255) );
	else
	{
	...
	if( m_pListOverItem[m_InsertedCount] )
		m_pListOverItem[m_InsertedCount]->SetColorKey( RGB(255, 0, 255) );
	else
	{
