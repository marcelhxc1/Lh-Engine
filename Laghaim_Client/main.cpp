...
const int MAX_CAPSYONG_TEXTURE = 3500;
...
DWORD	g_dwLangType = LANG_KOR;

DWORD	g_dwClientCountry = CTRY_KOR;

BOOL    g_bFixedWidthFont = TRUE;
...
void CMyD3DApplication::RestartGame()
{
	...
	::SetWindowText(pCMyApp->Get_hWnd(), "Laghaim");
}
	...
	{
		{
			...
			if( pFrame )
			{
				pFrame->SetColorKey(RGB(255, 0, 255));
				pFrame->Xpos = left;
...
void CMyD3DApplication::LoadingWorldImg(int percent)
{
	...
	if( pBackSur )
	{
		pBackSur->SetColorKey(RGB(255, 0, 255));
		...
		if( pTextSur )
		{
			pTextSur->SetColorKey(RGB(255, 0, 255));
...
BOOL CMyD3DApplication::LoadGameResources()
{
	...
	if( g_MatrixSvr != 0 )
	{
		...
		g_pWhangLolo1->Read( "data/effect/lolos/m_symbol_b.LOL" );

		g_pWhangLolo2 = new CLolos;
		g_pWhangLolo2->SetIndexedTexture( g_pCapsyongTexture );
		g_pWhangLolo2->Read( "data/effect/lolos/m_symbol_g.LOL" );
	...
	else
	{
		...
		g_pWhangLolo3 = g_pWhangLolo2 = new CLolos;
		g_pWhangLolo2->SetIndexedTexture( g_pCapsyongTexture );
...
{	
	...
	DeleteEffect_Fairy(SKILL_PET_RABBIT_GOLD);
	DeleteEffect_Fairy(SKILL_PET_RABBIT_MIX);
	DeleteEffect_Fairy(SKILL_PET_RABBIT_WHITE);
	...
	else if (GetCurWorld() == WORLD_SKY||GetCurWorld() ==WORLD_DUNGEON4)
	{
		ChangeHour(0);
		if( m_pWeatherMgr )
...
BOOL CMyD3DApplication::LoadWorldMap(int zone_index)
{
	...
	if( info->m_new_terrain )
	{
		...
		if( m_pFrustum )
		{
			...
			{
				m_pFrustum->InitObjFrustum( 2000.0f );
				m_pFrustum->InitFrustum( m_Fov, m_Aspect, m_zNear, 2000.0f );
			}
		}
	}
			...
			if( dwTempTick > 0 )
			{
				if(  (timeGetTime()%1000) > 450  )
				{
					...
					if( len )
					{
						...
						TempTextOut.Init(font_size, RGB(255, 0, 255), (g_pNk2DFrame->GetClientWidth()-len*(font_size/2)) / 2, g_pNk2DFrame->GetClientHeight()/2 - 100 );
	...
	if( m_pMp3Module[ 0 ] )
	{
		if( g_dwClientCountry == CTRY_JPN ) ///일본BGM 옛날껄로 돌리는 이벤트.
		{
			if( m_ProOption.bMusicOn ) // 옵션 체크를 그냥 여기서...
			{
				...
				if( m_PlaySndType == SIEGEWAR_SOUND || m_PlaySndType == DUNGEON8_SOUND || m_PlaySndType == DMITRON_SOUND
						|| m_PlaySndType == PEACEZONE_SOUND || m_PlaySndType == WARZONE_SOUND || m_PlaySndType == DUNGEON8_SOUND
						|| m_PlaySndType == FORLORN_SOUND
				  ) // mp3플레이 지역.
				{
				...
				else if( m_nCurWorldIdx == WORLD_DUNGEON4)
				{
					...
					if( m_PlaySndType != DUNGEON8_SOUND )
					{

						m_pMp3Module[ 0 ]->SwapSourceFilter("dungeon8.mp3");
						m_PlaySndType = DUNGEON8_SOUND;
					}
				}
				else if( m_nCurWorldIdx == WORLD_DMITRON)
				{
					...
					if( m_PlaySndType != DMITRON_SOUND )
					{
						m_pMp3Module[ 0 ]->SwapSourceFilter("Dmitron.mp3");
						m_PlaySndType = DMITRON_SOUND;
					}
				}
		...
		{
			if( m_ProOption.bMusicOn && m_nCurWorldIdx == WORLD_DUNGEON4)
			{
				if( !m_pMp3Module[ 0 ]->IsPlaying() )
					m_pMp3Module[ 0 ]->Play();
				if( m_PlaySndType != DUNGEON8_SOUND )
			...
			// nate 2005-08-31 : 드미트론 배경 음악
			else if( m_ProOption.bMusicOn && m_nCurWorldIdx == WORLD_DMITRON)
			{
				if( !m_pMp3Module[ 0 ]->IsPlaying() )
					m_pMp3Module[ 0 ]->Play();
				if( m_PlaySndType != DMITRON_SOUND )
			...
			else if( m_ProOption.bMusicOn && m_nCurWorldIdx == WORLD_DISPOSAL)
			{
				if( !m_pMp3Module[ 0 ]->IsPlaying() )
					m_pMp3Module[ 0 ]->Play();
				if( m_PlaySndType != FORLORN_SOUND )
				{
					m_pMp3Module[ 0 ]->SwapSourceFilter("Forlorn_Ruin.mp3");
					m_PlaySndType = FORLORN_SOUND;
				}
			}
			else if( m_ProOption.bMusicOn && 
					 (m_nCurWorldIdx == WORLD_FORLORN_S2 ||
					  m_nCurWorldIdx == WORLD_FORLORN_S3 ||
					  m_nCurWorldIdx == WORLD_FORLORN_S4) )
			{
...
//===============================================================
			...
			else if (Dist3PTo3P(m_vEye.x, m_vEye.y, m_vEye.z, m_vAt.x, fAtHeight, m_vAt.z) >= 120)  		//전에는 150이었다.
			{
				return;
			}
//===============================================================
...
bool CMyD3DApplication::SkillConditionCheck(int cur_skill)
{
	...
	case SKILL_A_LIFE_EXPANSION:
		...
		if( GET_ITEM_NUM( g_pRoh->m_Wearing[ WEARING_SHIELD ] ) != 1022 		// 1022 : 파술피리
				&& GET_ITEM_NUM( g_pRoh->m_Wearing[ WEARING_SHIELD ] ) != 3067 )	// 3067 : 파술피리
			return false;
		return true;
...
BOOL CMyD3DApplication::IsInsideMap(float x, float z)
{
	...
	else
	{
		...
		else if( m_nCurWorldIdx == WORLD_FORLORN_S2 || m_nCurWorldIdx == WORLD_FORLORN_S3 || m_nCurWorldIdx == WORLD_FORLORN_S4 )
		{
			return TRUE;
		}
		else
...
HRESULT CMyD3DApplication::CheckDeviceCaps()
{
	...
	if (!m_bCanDoMultitexture && !(bSrcColor && bDestColor) )
	{
		...
		MessageBox(NULL, "디바이스가 알파블렌딩을 할 수 없네요", "Warning", MB_OK);
		return E_FAIL;
	}
