...
//		if( atoi(strTemp1) != -1 )
		{
			...
			g_pDisplay->CreateSurfaceFromText( &m_RankList[i].pSurRank, m_Font, strTemp2, RGB(255, 0, 255), RGB(255,200,1000) );

			if( m_RankList[i].pSurRank )
				m_RankList[i].pSurRank->SetColorKey(RGB(255, 0, 255));

			// 영웅의 이름
			if( atoi(strTemp1) == -1 )
			{
				if( m_TotalPage > m_Page || (m_TotalPage == m_Page && num > 0 && num > i) )
					g_pDisplay->CreateSurfaceFromText( &m_RankList[i].pSurName, m_Font, "------------", RGB(255, 0, 255), RGB(255,200,1000) );
				else
					g_pDisplay->CreateSurfaceFromText( &m_RankList[i].pSurName, m_Font, " ", RGB(255, 0, 255), RGB(255,200,1000) );

			}
			else
				g_pDisplay->CreateSurfaceFromText( &m_RankList[i].pSurName, m_Font, strTemp1, RGB(255, 0, 255), RGB(255,200,1000) );

			if( m_RankList[i].pSurName )
				m_RankList[i].pSurName->SetColorKey(RGB(255, 0, 255));
